...
public partial class MainWindow : WindowEx
{
    ...
    private async void Window_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        if (AppSettings.Default.General.FirstOpen)
        {
            ...
            {
                s_logger.Error(ex, "Error while scanning custom folder: {0}",
        ...
        else
        {
            if (DateTime.Now - AppSettings.Default.LastTimeScanOsuDb > TimeSpan.FromDays(1))
            {
                ...
                {
                    s_logger.Error(ex, "Error while syncing osu!db: {0}", AppSettings.Default.General.DbPath);
        ...
        {
            s_logger.Error(ex, "Error while checking for update");
