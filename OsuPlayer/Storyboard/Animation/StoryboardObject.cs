...
using Milkitic.OsuPlayer.Storyboard.Util;
using SharpDX;
using D2D = SharpDX.Direct2D1;
using Gdip = System.Drawing;
using Mathe = SharpDX.Mathematics.Interop;

namespace Milkitic.OsuPlayer.Storyboard.Animation
{
    public class StoryboardObject : IDisposable
    {
        ...
        public StoryboardObject(Element element, Timing timing, D2D.RenderTarget target, Size2F vSize)
        {
            ...
            _element = element;
            _vSize = vSize;
            var path = Path.Combine(Core.StoryboardProvider.Directory, element.ImagePath);
            if (File.Exists(path))
            {
