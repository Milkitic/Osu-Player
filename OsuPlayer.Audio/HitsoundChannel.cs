...
public class HitsoundChannel : MultiElementsChannel
{
    ...
    public HitsoundChannel(string defaultFolder, LocalOsuFile osuFile, AudioPlaybackEngine engine, FileCache? cache = null)
        : this(defaultFolder, osuFile, Path.GetDirectoryName(osuFile.OriginPath)!, engine, cache)
    {
    }
    ...
    public override string Description => nameof(HitsoundChannel);
        ...
        ConcurrentBag<SoundElement> elements)
    {
        ...
        else // sliders
        {
            ...
            // ticks
            var ticks = obj.SliderInfo.GetSliderTicks();
            ...
            var trails = obj.SliderInfo.GetSliderSlides();
            var all = trails
                .Select(k => new
                {
                    offset = k.Offset,
        ...
        HashSet<string> waves)
    {
        ...
        var additionStr = itemAddition.ToHitsoundString(sampleStr);
        ...
        for (var i = 0; i < tuples.Count; i++)
        {
            ...
            {
                filePath = Path.Combine(_defaultFolder, fileNameWithoutExt + Information.WavExtension);
            }
            ...
            {
                filePath = Path.Combine(_defaultFolder, fileNameWithoutIndex + Information.WavExtension);
            }
